...
def text_check(text):
    py2str = (sys.version_info.major == 2 and isinstance(text, str))
...
example we can generate its string representation:
    ...
    nodes = list(map(remove_private, nodes))
...
def remove_private(n):
    _ks = [k for k in list(n.keys()) if k.startswith('_') or k == 'attrib']
...
def copy(n):
    m = El(n.tag, n.text, **dict(list(n.items())))
...
symbol_names = sorted(list(symbols.keys()), key=lambda s: len(s), reverse=True)
